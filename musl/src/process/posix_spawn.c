	...
	if (fa && fa->__actions) {
		...
		for (; op; op = op->prev) {
			...
			if (op->fd == p) {
				...
				if (ret < 0) goto fail;
				__syscall(SYS_cbn_close, p);
				p = ret;
			}
			switch(op->cmd) {
			case FDOP_CLOSE:
				__syscall(SYS_cbn_close, op->fd);
				break;
			case FDOP_DUP2:
				fd = op->srcfd;
				if (fd == p) {
				...
				break;
			case FDOP_OPEN:
				fd = __sys_cbn_open(op->path, op->oflag, op->mode);
				if ((ret=fd) < 0) goto fail;
				if (fd != op->fd) {
					if ((ret=__sys_dup2(fd, op->fd))<0)
						goto fail;
					__syscall(SYS_cbn_close, fd);
				}
				break;
			}
		}
...
fail:
	...
	ret = -ret;
	if (ret) while (__syscall(SYS_cbn_write, p, &ret, sizeof ret) < 0);
	_exit(127);
}
