...
static int printf_core(struct sprintf_buf *f, const char *fmt, va_list *ap, union arg *nl_arg, int *nl_type)
{
	char *a, *z, *s=(char *)fmt;
	unsigned l10n=0, fl;
	int w, p;
...
int vsprintf(char *f, const char *fmt, va_list ap)
{
	...
	va_copy(ap2, ap);
	if ((ret = printf_core(&buf, fmt, &ap2, nl_arg, nl_type)) < 0) return -1;
	return ret;
}
